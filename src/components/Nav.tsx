    ...
    <nav className="fixed top-0 inset-x-0 z-50 bg-slate-950/80 backdrop-blur-lg border-b border-slate-800/50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/#install"
            className="hidden sm:inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-400 text-slate-950 font-medium text-sm px-4 py-2 rounded-lg transition-all hover:shadow-lg hover:shadow-brand-500/25"
          >
        ...
        <div className="md:hidden bg-slate-950/95 backdrop-blur-lg border-b border-slate-800/50 px-4 py-4 space-y-3">
          ...
          <div className="pt-2 border-t border-slate-800 space-y-2">
            {session?.user ? (
              ...
              <button onClick={() => signIn('github')} className="block text-sm text-slate-400 hover:text-white py-2 w-full text-left">
                Sign In with GitHub
              </button>
            )}
            <Link
              href="/#install"
              className="inline-flex items-center gap-2 bg-brand-500 text-slate-950 font-medium text-sm px-4 py-2 rounded-lg w-full justify-center"
              onClick={() => setMobileOpen(false)}
            >
              {t('nav_install')}
            </Link>
